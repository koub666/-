...
$(function () {
  ...
  function renderWord(data, copy = true) {
    ...
    // Check if the mean only contains part of speech (like "vt.", "adj.", etc.)
    let is_only_pos = true;
    for (let i = 0; i < means.length; i++) {
      let mean = means[i].trim();
      if (mean && !/^[a-zA-Z]+\.?$/.test(mean)) {
        is_only_pos = false;
        break;
      }
    }
    
    // If only part of speech, try to get full translation from dict.cn
    if (is_only_pos && data.mean.length < 10) {
      // Create a placeholder for translation
      mean_content += "<p class='translating'>正在获取完整翻译...</p>";
      $.template("mean", mean_content);
      $("#tmpl-content").empty();
      $.tmpl("mean").appendTo($("#tmpl-content"));
      
      // Fetch translation from external API
      $.ajax({
        ...
        success: function(response) {
          if (response.status === 200 && response.data) {
            ...
            let clean_translation = $(response.data).text().trim();
            
            // Update the display with the fetched translation in a box
            let fetched_content = data.webster ? '<a style="color: red;">  𝓦</a>' : "";
            fetched_content += `
              <p>${data.mean}</p>
              <div class="translation-box">
                <div class="translation-label">完整翻译：</div>
                <div class="translation-content">${clean_translation}</div>
              </div>
            `;
            $.template("mean", fetched_content);
            $("#tmpl-content").empty();
            $.tmpl("mean").appendTo($("#tmpl-content"));
          }
        }
      ...
      // Normal display for words with full translation
      for (let i = 0; i < means.length; i++) {
